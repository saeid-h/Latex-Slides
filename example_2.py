...
def ROI_box_gt(gt, rgb_path, thickness=3):
    th = thickness
    x = 192; y = 192
    rgb = gt[:,:,np.newaxis]
    ...
    rgb[x-th:x+128+th,y-th:y+th,:] = [0,255,0]
    rgb[x-th:x+128+th,y+160-th:y+160+th,:] = [0,255,0]
    temp_save = os.path.join ('tmp', 'gt_ROI_'+rgb_path.split(os.sep)[-1])
    cv_io.save(temp_save, rgb.astype(np.uint8))
    return temp_save
...
if __name__ == "__main__":
    ...
    for i, image_tuple in enumerate(images):
        ...
        items = ['Miniumum Depth in GT: {:6.2f}'.format(min_depth_total)]
        items.append('Maximum Depth in GT: {:6.2f}'.format(max_depth_total))
        stat_total = Items(items)
        
        gt = gt[192:192+128,192:192+160]
        min_depth_ROI = np.min(gt[gt>0])
        max_depth_ROI = np.max(gt)
        items = ['Miniumum Depth in GT (ROI): {:6.2f}'.format(min_depth_ROI)]
        items.append('Maximum Depth in GT (ROI): {:6.2f}'.format(max_depth_ROI))
        ...
        table_data += [[mask_final_img, stat_total, stat_ROI]]
        table_data += [['Final Mask', 'GT Stat.', 'GT Mask Stat.']]
